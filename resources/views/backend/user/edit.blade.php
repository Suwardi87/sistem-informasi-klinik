...
<div id="main">
        ...
        <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
            <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                ...
                <li class="breadcrumb-item active" aria-current="page"><a
                        href="{{ route('backend.provinsi.index') }}">Data Provinsi</a></li>
                <li class="breadcrumb-item active" aria-current="page"><a
                        href="{{ route('backend.provinsi.edit', $provinsi->uuid) }}">@yield('title')</a></li>
                </ol>
        </nav>
        <div class="d-flex justify-content-between w-100 flex-wrap">
            <div class="mb-3 mb-lg-0">
                <h1 class="h4">@yield('title')</h1>
                <p class="mb-0">Edit Data Provinsi</p>
            </div>
            <div>
                <a href="{{ route('backend.provinsi.index') }}" class="btn btn-outline-primary"><i
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('backend.provinsi.update', $provinsi->uuid ?? '') }}" method="POST" id="formUpdateProvinsi" class="needs-validation">
                @csrf
                @method('PUT')
                 <input type="hidden" id="uuid" value="{{ $provinsi->uuid }}">
                <div class="row">
                    <div class="col-md-12">
                        <div class="my-3">
                            <label for="nama" class="form-label">Nama Provinsi</label>
                            <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama"
                                name="nama" value="{{ old('nama', $provinsi->nama) }}" required>
                            @error('nama')
